...
def command_run(message: Any, hover: Any, command: str) -> RTextBase:
    ...
    return fancy_text.set_hover_text(hover).set_click_event(
...
def show_help_message(source: CommandSource):
    source.reply(
        tr("help_message", name=metadata.name, version=metadata.version, prefix=PREFIX)
    )
...
def copy_worlds(src: str, dst: str):
    for world in config.world_names:
        ...
        def filter_ignore(path, files):
            ...
            for file in files:
                if file == "session.lock" and config.ignore_session_lock:
                    ignore.append(file)
                if file in carpet_conf and config.ignore_carpet_conf:
                    ignore.append(file)
            return ignore
        ...
        if os.path.isdir(src_path):
            ...
            server_inst.logger.warning(
                "{} does not exist while copying ({} -> {})".format(
    ...
    for world in config.world_names:
        ...
        if os.path.isdir(target_path):
            ...
            ServerInterface.get_instance().logger.warning(
                "{} does not exist while removing".format(target_path)
            )
...
def sync(source: CommandSource):
    ...
    source.get_server().say(
        command_run(
            tr("sync.confirm_hint", PREFIX),
            tr("sync.confirm_hover"),
            "{0} confirm".format(PREFIX),
        )
        + ", "
        + command_run(
            tr("sync.abort_hint", PREFIX),
            tr("sync.abort_hover"),
    ...
    if config.backup:
        if constants.QBM_PID in server.get_plugin_list():
            ...
            server.dispatch_event(
                constants.TRIGGER_BACKUP_EVENT,
                (
...
def register_command(server: PluginServerInterface):
    def get_literal_node(literal):
        return Literal(literal).requires(
            lambda src: src.has_permission(config.permission_level),
